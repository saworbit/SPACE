...
- **Phase 4 "View" projection mount + zone bridge** - Added `spacectl put` + `spacectl project mount` (and `--zone`) for simulation-first, zero-install projection into a `content` file plus optional zone replication via `crates/federation` (`scripts/test_phase4_projection.sh` validates the flow).
