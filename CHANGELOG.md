...
- **Code-review simplification pass on the dedup/scrub/foundry diff** (`crates/dedup/src/lib.rs`, `crates/common/src/traits.rs`, `crates/common/src/scrub.rs`, `crates/capsule-registry/src/scrub_executor.rs`, `crates/capsule-registry/src/pipeline/legacy.rs`, `crates/foundry/src/backend/device.rs`)
  - **`Deduper::hash_content_with_algo` is now a required trait method.** The previous default impl silently dropped `algo` and called `hash_content`, recreating the exact cross-policy collision bug the algo parameter exists to prevent. `Blake3Deduper` already provides the correct override; new `Deduper` impls now get a compile-time error rather than a silently-wrong key
  - **Two-scheme hash verification moved into the `dedup` crate** as `verify_content_hash(expected, data, algo) -> VerifyOutcome { Matched, LegacyMatched, Mismatched }`. `ScrubExecutor` no longer imports both `hash_content` and `hash_content_with_algo` — the legacy-fallback knowledge lives at the right abstraction level and is reusable by any future read-path verifier. Three new unit tests in `dedup` cover the three outcomes
  - **`ScrubReport::legacy_hash_hits: u64`** field added (serde-defaulted) — counts segments that verified clean only via the pre-fix bare-BLAKE3 fallback. Operators watch this counter trend to zero before retiring the compatibility window; not classified as an error since these segments are integrity-clean
  - **`decompress_segment(segment, policy, decrypted)` helper extracted** in `pipeline::legacy` to deduplicate the two byte-identical 23-line decompress blocks in `read_capsule` and `read_range`. The blocks encoded a load-bearing safety invariant ("trust `segment.compressed`, not `policy.compression`"); future fixes to that invariant land once instead of twice
  - **Foundry `DirectIoDevice` lock pattern annotated as stub-only** with `TODO(foundry-direct-io)` warning against copying the seek-based serialization into production code paths
  - Test narration comment removed from `deep_scrub_legacy_segment_with_bare_hash_passes`; `legacy_hash_hits == 1` asserted there to lock in the counter behavior
  - Verification: `cargo build --workspace`, `cargo test -p dedup -p capsule-registry -p foundry -p pipeline`, `cargo clippy -p dedup -p common -p capsule-registry -p foundry -p pipeline --all-targets -- -D warnings`
...
- **Cross-policy dedup collision could return mismatched plaintext** (`crates/dedup/src/lib.rs`, `crates/capsule-registry/src/pipeline/legacy.rs`, `crates/pipeline/src/lib.rs`, `crates/capsule-registry/src/scrub_executor.rs`)
  ...
  - Fix: added `hash_content_with_algo(data, algo)` and a corresponding `Deduper::hash_content_with_algo` trait method (initially defaulted to fall back to `hash_content`; the default was removed in the follow-up simplification pass above so new `Deduper` impls cannot silently reintroduce the bug). Both legacy pipeline write paths (sync `write_capsule_with_policy`, async `prepare_segment`) and the modular pipeline (`pipeline::Pipeline::write_capsule` and `write_capsule_in_zone`) now domain-separate the dedup key with a versioned BLAKE3 prefix `b"space.dedup.v1\0algo:" || algo || b"\0" || data`
  - **Scrub compatibility window** (added 2026-05-08 after a second review pass; subsequently moved into `dedup` as `verify_content_hash`): the legacy write paths always populated `Segment::compression_algo` with non-empty values (`"identity"`, `"lz4:N"`, `"zstd:N"`), so distinguishing pre-fix from post-fix segments by the algo field is impossible — the previous "empty algo means legacy" check was dead code. `dedup::verify_content_hash` tries the algo-aware hash first and falls back to bare `hash_content` for legacy data; `ScrubExecutor` calls the verifier, emits a `tracing::warn!` on `LegacyMatched`, and bumps `ScrubReport::legacy_hash_hits` so operators can observe the migration. The fallback can be removed once that counter trends to zero. The dedup write index does **not** accept the legacy form — only scrub verification does — so the fallback cannot reintroduce cross-policy collisions
