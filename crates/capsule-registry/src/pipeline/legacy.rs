...
impl LegacyPipeline {
    ...
    pub fn set_mesh_node(&mut self, mesh_node: std::sync::Arc<scaling::MeshNode<CapsuleRegistry>>) {
        self.mesh_node = Some(mesh_node);
    }
}
...
impl PipelineStrategy for LegacyPipeline {
    ...
    async fn write_capsule(&self, data: &[u8], policy: &Policy) -> Result<CapsuleId> {
        self.write_capsule_with_policy_async(data, policy).await
    }

    #[cfg(not(feature = "pipeline_async"))]
    async fn write_capsule(&self, data: &[u8], policy: &Policy) -> Result<CapsuleId> {
        self.write_capsule_with_policy(data, policy)
    }
