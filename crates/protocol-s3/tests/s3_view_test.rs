...
fn test_s3_put_and_get() {
    ...
    // PUT object
    let capsule_id = s3.put_object("test-bucket", "hello.txt", test_data.clone()).unwrap();
...
fn test_s3_multiple_objects() {
    ...
    s3.put_object("bucket1", "file1.txt", b"Content 1".to_vec()).unwrap();
    s3.put_object("bucket1", "file2.txt", b"Content 2".to_vec()).unwrap();
    s3.put_object("bucket2", "file3.txt", b"Content 3".to_vec()).unwrap();
...
fn test_s3_large_object() {
    ...
    s3.put_object("test", "large.bin", large_data.clone()).unwrap();
