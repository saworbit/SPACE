...
pub fn encrypt_segment(
    ...
    // Create metadata
    let metadata = EncryptionMetadata::new_xts(
        key_version,
        tweak,
...
pub fn decrypt_segment(
    ...
    let tweak = metadata.require_tweak()
        .map_err(|e| EncryptionError::CorruptedMetadata(e.to_string()))?;
    
    // Verify length matches
    let expected_len = metadata.ciphertext_len
...
mod tests {
    ...
    fn test_decrypt_unencrypted_metadata_fails() {
        ...
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), EncryptionError::MissingMetadata));
