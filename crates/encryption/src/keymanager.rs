...
impl KeyManager {
    ...
    pub fn from_env() -> Result<Self> {
        let hex_key = std::env::var("SPACE_MASTER_KEY")
            .map_err(|_| EncryptionError::InvalidConfiguration(
                "SPACE_MASTER_KEY environment variable not set".to_string()
            ))?;
        
        let bytes = hex::decode(&hex_key)
            .map_err(|e| EncryptionError::InvalidConfiguration(
                format!("Invalid hex in SPACE_MASTER_KEY: {}", e)
            ))?;
...
mod tests {
    ...
    fn test_from_env_missing() {
        ...
        assert!(matches!(result.unwrap_err(), EncryptionError::InvalidConfiguration(_)));
    }
    ...
    fn test_from_env_invalid_hex() {
        ...
        assert!(result.is_err(), "Expected error for invalid hex, got: {:?}", result);
        if let Err(e) = result {
    ...
    fn test_from_env_wrong_length() {
        ...
        assert!(matches!(result.unwrap_err(), EncryptionError::InvalidKeyLength { .. }));
    }
}