    ...
    if (viewMode === 'wear') {
      ...
      switch (zone.status) {
        case 'sequential':
          return 'bg-blue-500';
        case 'cold':
          return 'bg-nebula-purple';
        case 'gc-pressure':
          return 'bg-supernova-orange';
        default:
          return 'bg-white/10';
      }
    }
            ...
            <div className="space-y-4">
              <div>
                ...
                <div
                  className={`inline-flex items-center gap-2 px-3 py-1 rounded ${
                    selectedZone.status === 'sequential'
                      ? 'bg-blue-500/20 text-blue-400'
                      : selectedZone.status === 'cold'
                ...
                >
                  ...
                  <span className="text-sm">
                    {selectedZone.status.toUpperCase().replace('-', ' ')}
                  </span>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="text-white font-mono text-sm">
                  0x{selectedZone.writePointer.toString(16).toUpperCase()}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1 h-2 bg-white/10 rounded-full overflow-hidden">
                    <motion.div
                      className={`h-full ${
                        selectedZone.wear < 30
                          ? 'bg-green-500'
                          : selectedZone.wear < 60
                            ? 'bg-yellow-500'
                            : selectedZone.wear < 80
                              ? 'bg-orange-500'
                              : 'bg-red-500'
                      }`}
                      initial={{ width: 0 }}
