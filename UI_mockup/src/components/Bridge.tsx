  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setNodes(prev =>
        prev.map(node => ({
          ...node,
          latency: node.status === 'lagging' ? 40 + Math.random() * 20 : Math.random() * 20,
          ...
          .map((node, index) => {
            ...
            return (
              <circle
                ...
                strokeWidth="1"
                className={
                  node.status === 'lagging' ? 'text-supernova-orange/20' : 'text-neon-cyan/10'
                }
                strokeDasharray="4 4"
              />
            );
          ...
          .map((node, index) => {
            ...
            return (
              <motion.line
                ...
                strokeWidth="1"
                className={
                  node.status === 'healthy' ? 'text-neon-cyan/30' : 'text-supernova-orange/30'
                }
            ...
            >
              <div
                className={`w-full h-full rounded-full border-2 flex items-center justify-center ${
                  node.status === 'healthy'
                    ? 'bg-nebula-purple/20 border-nebula-purple'
                    : node.status === 'lagging'
                      ? 'bg-supernova-orange/20 border-supernova-orange glow-orange'
