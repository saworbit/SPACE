...
> **Implementation note.** The default `CryptoProfile` path matches this claim: the XTS tweak is deterministically derived from the BLAKE3 content hash, so identical plaintext under the same key version produces identical ciphertext, and the on-disk segment is safely shared across capsules via the hash-keyed content store. The `advanced-security` + `CryptoProfile::HybridKyber` path diverges — ML-KEM wraps a per-capsule/segment XTS key (mixing `capsule_id` and logical `segment_index` into the derivation) and the tweak is mixed with a per-segment nonce. The current read path re-derives that material from the *reading* capsule's context, so a dedup hit would produce the wrong wrapped key. `Policy::validate()` rejects `HybridKyber + dedupe = true` at the pipeline boundary (`PolicyError::HybridKyberDedupConflict`) to prevent silent data inaccessibility; lifting the restriction requires either keying the dedup index on the crypto context or storing the originating derivation context for use on read. See [implementation/DEDUP_IMPLEMENTATION.md](implementation/DEDUP_IMPLEMENTATION.md) and [implementation/ENCRYPTED_DEDUP_FLOW.md](implementation/ENCRYPTED_DEDUP_FLOW.md) for the lookup model.
